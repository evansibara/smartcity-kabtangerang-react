...
export default function SmartSociety() {
  ...
  useEffect(() => {
    const fetchInovasi = async () => {
      try {
        setLoading(true);
        const response = await apiEndpoints.inovasi.getAll('Smart Society');
    ...
    <div className="explore-page">
      <section className="explore-hero">
        <div className="explore-hero-content">
          <h1 className="explore-hero-title">Smart Society</h1>
                  ...
                  {paginatedArtikel.map(item => (
                    <article key={item.id} className="explore-card">
                      {item.category === 'inovasi' ? (
                        <div className="explore-card-image" onClick={() => handleImageClick(item)}>
